<template>
  <div class="my-car-break-order-page">
    ...
    <scroller>
      <div class="wrapper" v-show="isShowPage">
        ...
        <div class="order-detail-list">
          <div class="detail-item" v-show="handleOrderItemShow(item)" v-for="(item, index) in userOrderList" :key="index">
            <h3>
            ...
            <div class="delete-btn on" v-if="0 == item.price * 1">处理中</div>
            <div class="delete-btn" v-else-if="item.status == 0 ? true: false" v-on:click.stop="goOrderPayPage(item)">去支付</div>
            <i class="icon icon-del" v-if="item.status == 0 ? true: false" @click="cancelOrder(item, index)"></i>
          </div>
        </div>
