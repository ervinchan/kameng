...
export default {
  ...
  methods: {
    ...
    handleIntegral () {
      ...
      .signIn()
      .then((res) => {
        this.integral = _.get(res.data, 'integral') ?  _.get(res.data, 'integral') * 1 : 0;
    ...
    handleSumbit () {
      ...
      if (self.integral < self.formTemp.goods.integral * 1) {
        self.$toast('积分不足，无法兑换');
        return;
      }
    ...
    integralGoodsInfo () {
      ...
      .then((res) => {
        if (1 === res.code) {
          let data = res.data;
          if (self.integral < data.integral * 1) {
            self.$toast('积分不足，无法兑换');
          }

          self.formTemp.goods = data;
        }
      });
    },
